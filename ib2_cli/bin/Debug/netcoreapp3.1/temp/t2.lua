...
local function gBits8()
    local F = BitXOR(Byte(ByteString, Pos, Pos), 158);
    Pos = Pos + 1;
    return F;
end;
...
local function gString(Len)
	...
	local FStr = {}
	for Idx = 1, #Str do
		FStr[Idx] = Char(BitXOR(Byte(Sub(Str, Idx, Idx)), 158))
	end
...
local function Wrap(Chunk, Upvalues, Env)
	...
	return function(...)
		...
		while true do
			...
			Enum		= Inst[1];if Enum <= 3 then if Enum <= 1 then if Enum > 0 then do return end;else Stk[Inst[2]] = Inst[3];end; elseif Enum == 2 then do return end;else 
local A = Inst[2]
Stk[A](Stk[A + 1])
end; elseif Enum <= 5 then if Enum == 4 then Stk[Inst[2]] = Inst[3];else Stk[Inst[2]]=Env[Inst[3]];end; elseif Enum == 6 then 
local A = Inst[2]
Stk[A](Stk[A + 1])
else Stk[Inst[2]]=Env[Inst[3]];end;
